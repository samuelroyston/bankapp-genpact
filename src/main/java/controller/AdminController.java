...
public class AdminController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * private String generateAccountNo() { // Generate a 10-digit account number //
		 * Format: YYYY + MM + 5 random digits java.time.LocalDate currentDate =
		 * java.time.LocalDate.now(); int year = currentDate.getYear(); int month =
		 * currentDate.getMonthValue(); int randomDigits = (int) (Math.random() *
		 * 100000); // 5-digit random number
		 * 
		 * return String.format("%04d%02d%05d", year, month, randomDigits); }
		 * 
		 * private String generateTempPassword() { // List of common, easy-to-remember
		 * words String[] words = {"cat", "dog", "sun", "moon", "star", "tree", "book",
		 * "bird", "fish", "home"};
		 * 
		 * // Generate a random number between 1000 and 9999 int randomNum = 1000 +
		 * (int)(Math.random() * 9000);
		 * 
		 * // Select a random word from the array String word =
		 * words[(int)(Math.random() * words.length)];
		 * 
		 * // Capitalize the first letter of the word word = word.substring(0,
		 * 1).toUpperCase() + word.substring(1);
		 * 
		 * // Combine the word and the number return word + randomNum; }
		 */
    private String generateAccountNo() {
        return "Berlin" + new Random().nextInt(999999999);
    }
